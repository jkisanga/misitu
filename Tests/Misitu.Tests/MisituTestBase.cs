...
namespace Misitu.Tests
{
    public abstract class MisituTestBase : AbpIntegratedTestBase<MisituTestModule>
    {
        ...
        protected void UsingDbContext(Action<MisituDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<MisituDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<MisituDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<MisituDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<MisituDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MisituDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<MisituDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MisituDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<MisituDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<MisituDbContext, Task<T>> func)
        {
            T result;
